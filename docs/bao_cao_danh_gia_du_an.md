...
## 2. DANH SÁCH CÁC CÔNG VIỆC ĐÃ THỰC HIỆN
...
### 2.2. Xây dựng Data Schemas & Core Tools (`tools/` & `src/tools.py`)
...
2. **Tool 2: `search_candidates` ([tools/search.py](file:///e:/Documents/GitHub/K4-D03-2A202601140-NguyenThanhTung/tools/search.py))**:
   - **Mock Database**: Thiết lập cơ sở dữ liệu gồm 18 hồ sơ ứng viên chuẩn hóa đa dạng độ tuổi, sở thích, khu vực.
...
### 2.3. Agent Core & Guardrails (`src/ai_levels/` & `src/prompts.py`)
- **System Prompt & Persona ([src/prompts.py](file:///e:/Documents/GitHub/K4-D03-2A202601140-NguyenThanhTung/src/prompts.py))**: Đóng vai "Bà Mối AI" tinh tế, ấm áp.
- **Interactive Information Gathering Loop**:
...
- **Interactive CLI Loop**: Hỗ trợ chạy trực tiếp qua bàn phím terminal.
...
- **Level 2 ([level2_llm_chatbot.py](file:///e:/Documents/GitHub/K4-D03-2A202601140-NguyenThanhTung/src/ai_levels/level2_llm_chatbot.py))**: Baseline LLM Chatbot sử dụng Google Gemini 3.5 Flash Lite với phanh kiểm tra đầu vào và xử lý ngoại lệ fallback.
- **Level 3 ([level3_reactive_agent.py](file:///e:/Documents/GitHub/K4-D03-2A202601140-NguyenThanhTung/src/ai_levels/level3_reactive_agent.py))**: ReAct Agent với chuỗi suy luận `Thought -> Action -> Observation`, PII Redaction và Slot Filling Loop.
- **Level 4 ([level4_autonomous_agent.py](file:///e:/Documents/GitHub/K4-D03-2A202601140-NguyenThanhTung/src/ai_levels/level4_autonomous_agent.py))**: Autonomous Agent tự lên kế hoạch (Planning 3 bước), duy trì bộ nhớ (Memory), phanh Planner Fallback và tự đánh giá (Evaluation).
...
| :---: | :--- | :--- | :--- | :--- | :--- |
| **#1** | **Đơn giản (Chỉ cần LLM)**<br>*"Tiêu chuẩn mối quan hệ lành mạnh là gì?"* | Khớp từ khóa chào hỏi/từ chối do không có từ khóa mối quan hệ. | Gemini 3.5 Flash Lite sinh 5 lời khuyên sâu sắc về tôn trọng, giao tiếp, tin tưởng. | Trả lời trực tiếp bằng văn phong Bà Mối AI ấm áp, không cần gọi tool. | Lên kế hoạch tự chủ tổng quát. |
...
[Execution]: Call Tool: search_candidates['Nữ', 22-28, 'Hà Nội']
[Observation]: Đã tìm thấy ứng viên tốt nhất: Ngọc Bích (25 tuổi, Hà Nội) - Score: 64.2%
...
[Execution]: Call Tool: calculate_compatibility['User', 'Ngọc Bích']
[Observation]: Điểm tương thích tổng hợp: 69.8/100. Điểm mạnh: Cùng sống tại Hà Nội, Độ tuổi rất hợp nhau (chênh lệch 1 tuổi), Tỷ lệ chiều cao chuẩn
...
[Execution]: LLM Strategic Generation (GeminiProvider - Gemini 3.5 Flash Lite)
[Observation]: Kịch bản hẹn hò:
Nam và Ngọc Bích hẹn hò tại một quán cà phê ở Hà Nội. Họ cùng nhau tận hưởng không khí lãng mạn và thưởng thức cà phê. Buổi hẹn hò kết thúc với một buổi nghe nhạc indie.
...
2. **Google Gemini Integration**: Mô hình `gemini-3.5-flash-lite` hoạt động mượt mà, phản hồi phản ứng nhanh, suy luận ReAct chuẩn xác.
