"""
🤖 AI MATCHMAKING AGENT CORE (`agent.py`)
Tích hợp toàn bộ hệ thống AI Agent Ghép Đôi ("Bà Mối AI") với Interactive CLI Loop,
Information Gathering Loop, Intent Analysis, Slot Filling và System Guardrails.
"""
...
class MatchmakingAgent:
    """
    Core AI Matchmaking Agent quản lý hội thoại, trích xuất Intent & Slots,
    kiểm soát thông tin thiếu và thực thi Tool có Guardrails.
    """
    ...
    def reset_state(self):
        """Reset sạch sẽ 100% toàn bộ trạng thái và lịch sử hội thoại"""
    ...
    def reset_current_task(self):
        """Reset các tham số slot của yêu cầu hiện tại để chuẩn bị cho câu prompt mới"""
    ...
    def extract_intent_and_slots(self, user_input: str) -> Dict[str, Any]:
        """
        Trích xuất Intent (SEARCH / COMPATIBILITY) và cập nhật các tham số thực tế do người dùng cung cấp.
        KHÔNG tự ý điền tham số mặc định để đảm bảo kích hoạt Vòng lặp hỏi thông tin thiếu (Slot Filling).
        """
        ...
        if intent == "SEARCH":
            ...
            # Gender Extraction (chỉ gán khi thực sự có trong văn bản)
    ...
    def check_missing_parameters(self) -> Tuple[bool, List[str]]:
        """
        Kiểm tra các tham số còn thiếu theo quy định trong Master System Prompt:
        - SEARCH: Giới tính mong muốn, Độ tuổi/khoảng tuổi, Vị trí (Tỉnh/Thành), Sở thích/Mong muốn.
        - COMPATIBILITY: Đầy đủ thông tin của cả 2 đối tượng.
        """
        intent = self.state["intent"]
        missing = []
    ...
    def process_message_llm_react(self, user_input: str) -> str:
        """
        Thực thi ReAct Loop hoàn toàn bằng LLM (LLM ReAct Tool Calling):
        LLM tự suy luận (Thought), tự chọn Tool và tham số (Action),
        nhận kết quả thực thi Tool (Observation), và tự tổng hợp Final Answer.
        """
        ...
        while iterations < MAX_ITERATIONS:
            ...
            # Kiểm tra nếu LLM bị lỗi API key hoặc đang dùng Mock fallback offline
            if any(err in llm_out for err in ["[Groq Error]", "[Gemini Error]", "[OpenAI Error]", "[Anthropic Error]", "[OpenRouter Error]", "🤖 [Mock Provider]"]):
                return None  # Kích hoạt Deterministic Guardrail Fallback
            
            # Trích xuất Action từ LLM ReAct output (hỗ trợ cả backticks `tool_name[...]`)
            ...
            if action_match:
                ...
                if tool_name in AVAILABLE_TOOLS:
                    ...
                    # Lưu bước kết quả trả về từ công cụ (Observation)
            ...
            # Nếu LLM đã đưa ra phản hồi hoặc Final Answer (không còn Action)
    ...
    def process_message(self, user_input: str) -> str:
        ...
        1. Thử gọi LLM ReAct Tool Calling trực tiếp (True LLM ReAct Agent).
        2. Nếu LLM chưa cấu hình API key/Mock -> Tự động chuyển qua Rule-Based Slot Filling Guardrail System.
        """
        # 1. Ưu tiên thực thi True LLM ReAct Tool Calling nếu đã cấu hình LLM API Key
        ...
        # 2. Fallback: Rule-Based Intent & Slot Filling Guardrail Loop
        text = user_input.lower()
        ...
        # Trích xuất intent và điền slots từ tin nhắn hiện tại
        self.extract_intent_and_slots(user_input)
        is_missing, missing_params = self.check_missing_parameters()

        # Bước 2: State & Guardrail Check (MAX_INFO_GATHERING_TURNS Limit)
        ...
        # 🛑 TRƯỜNG HỢP THIẾU PARAMETERS -> KHÔNG GỌI TOOL, HỎI BỔ SUNG KHÉO LÉO
        if is_missing:
        ...
        # ✅ TRƯỜNG HỢP ĐÃ ĐỦ PARAMETERS -> GỌI TOOL VỚI PHANH AN TOÀN TOOL_CALL_LIMIT
        tool_call_count = 0
        tool_res = None
        ...
        # Đánh dấu task đã hoàn tất
        self.state["is_task_complete"] = True
        return final_answer

    def format_persona_response(self, intent: str, tool_res: Dict, is_max_turn_fallback: bool = False) -> str:
        """Đóng vai Bà Mối AI văn phong tự nhiên, ấm áp, có emoji"""
        lines = []
        if is_max_turn_fallback:
            lines.append("⚠️ *Bà Mối đã tổng hợp thông tin hiện có để gợi ý ngay cho bạn đây:*")

        if intent == "SEARCH":
            candidates = tool_res.get("candidates", [])
            is_relaxed = tool_res.get("is_relaxed_search", False)
